...
int Application5::Initialize()
{
	...
	for (int i = 0; i < AAKERNEL_SIZE + 1; i++) {
		...
		*/
		...
		nameListShift[0] = GZ_AAKERNEL_SHIFTX;
		nameListShift[1] = GZ_AAKERNEL_SHIFTY;
		float zero = 0;
		if (i == AAKERNEL_SIZE) {
			valueListShift[0] = (GzPointer)&zero;
			valueListShift[1] = (GzPointer)&zero;
		}
		else {
	...
	*/ 
	...
	for (int i = 0; i < m_nWidth * m_nHeight; i++) {
		...
		for (int j = 0; j < AAKERNEL_SIZE; j++) {
			red += (float)m_pRender[j]->pixelbuffer[i].red * AAFilter[j][2];
			green += (float)m_pRender[j]->pixelbuffer[i].green * AAFilter[j][2];
			blue += (float)m_pRender[j]->pixelbuffer[i].blue * AAFilter[j][2];
		}

		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].red = (short)(red);
		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].green = (short)(green);
		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].blue = (short)(blue);
	}
